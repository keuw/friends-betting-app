...
- Public ledgers pair free-text search with pressed-state status filters, keep
  ...
  records match.
- The matched-bet ledger opens on `Pending`, showing every unresolved bet
  across the friend group. Resolved and Voided history remain explicit counted
  filters, while All restores the complete ledger.
- The `My live bets` score-strip metric is a compact drill-down action, not a
  ...
  destination always returns to `Pending`, and the other two score metrics stay
  noninteractive.
